# =============================================================================
# 06_main.R  (simulation2)
# 메인 실행 스크립트 — 여러 조건을 순서대로 실행
#
# simulation2 변경:
#   - 조건 코드: 8자리 2자리 고정 형식 (예: "01030201")
#   - IV 수준: IV1=7, IV2=10, IV3=4, IV4=1 → 총 280 조건
#   - output/ 경로: simulation2/output/
#
# 실행 전 준비사항:
#   1. 최초 1회: source("R/01_seed_generator.R") 으로 시드 테이블 생성
#   2. 아래 [사용자 설정 영역] 확인 후 실행
# =============================================================================

# ╔══════════════════════════════════════════════════════════════════════════╗
# ║                       사용자 설정 영역                                    ║
# ╠══════════════════════════════════════════════════════════════════════════╣
COND_CODES_OVERRIDE <- NULL  # NULL = ALL_COND_CODES (모든 조건 실행)
                             # 특정 조건만 실행하려면:
                             #   COND_CODES_OVERRIDE <- c("01010101", "02030201")
                             #   자리 01-02: IV1 인덱스 (sf4 값: 3.00~3.80, 7 수준)
                             #   자리 03-04: IV2 인덱스 (간격: 1.4~0.2, 10 수준)
                             #   자리 05-06: IV3 인덱스 (평균: 0~1.5, 4 수준)
                             #   자리 07-08: IV4 인덱스 (01=정규분포, 1 수준)
...
# ── 조건 코드 결정 ───────────────────────────────────────────────────────────
COND_CODES <- if (is.null(COND_CODES_OVERRIDE)) ALL_COND_CODES else COND_CODES_OVERRIDE
